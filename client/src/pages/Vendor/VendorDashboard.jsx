...
export default function VendorDashboard() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const commDeducted = (revenue * commRate) / 100;
      const net = revenue - commDeducted;
  ...
  const handleProductSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        seller_id: user.id,
        // variant attributes & SEO details
        specs: {
          sku: productForm.sku || `SKU-${Math.random().toString(36).substring(2,8).toUpperCase()}`,
          meta_title: productForm.metaTitle || productForm.name,
          meta_desc: productForm.metaDescription || productForm.description,
        }
      };
  ...
  const resetProductForm = () => {
    setProductForm({
      ...
      img: "", description: "", stock: "", colors: "", sizes: "", sku: "", metaTitle: "", metaDescription: "", deliveryDays: 3
    });
  };
  ...
  if (vendorData && vendorData.status !== "approved") {
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center pt-24 px-6">
        <div className="max-w-md w-full bg-[#111] border border-white/10 rounded-2xl p-8 text-center shadow-2xl">
          ...
          <h2 className="display text-2xl font-bold mb-4">Application Under Review</h2>
          ...
          <div className="p-4 bg-white/5 rounded-xl border border-white/5 text-xs text-white/40 mb-6">
          ...
          <button 
            ...
            className="btn-outline px-6 py-2.5 rounded-xl text-xs tracking-widest uppercase text-white/70 hover:text-white"
          >
