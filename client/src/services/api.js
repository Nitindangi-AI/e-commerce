...
export const authAPI = {
  register: async ({ firstName, lastName, email, password, phone, role, storeName, panCard, gstNumber, bankAccount, aadharNumber }) => {
    ...
    if (userId) {
      ...
      // If registered as vendor, insert pending vendor onboarding row
      if (userRole === 'vendor') {
...
export const orderAPI = {
  create: async ({ orderItems, shippingAddress, paymentMethod, couponCode, discount }) => {
    ...
    // Generate order ID
    const orderId = `LX-${Date.now().toString(36).toUpperCase()}${Math.random().toString(36).slice(2, 5).toUpperCase()}`;
...
function normalizeOrder(o) {
  ...
  const returnEligible = isDelivered && o.return_status === 'none' && delivered && (Date.now() - delivered.getTime() <= fiveDaysMs);
  const returnDaysRemaining = delivered && o.return_status === 'none'
    ? Math.max(0, Math.ceil((fiveDaysMs - (Date.now() - delivered.getTime())) / (24 * 60 * 60 * 1000)))
    : 0;
