...
exports.createOrder = asyncHandler(async (req, res) => {
  ...
  for (const item of orderItems) {
    const productRes = await db.query("SELECT * FROM products WHERE id = $1", [item.product]);
  ...
  const orderInsertValues = [
    req.user._id.toString(), orderId, JSON.stringify(shippingAddress), selectedPayment, paymentStatus, subtotal, shippingCost, discountAmount, couponCode || null, totalAmount, estimatedDelivery, orderNumber
  ];
  ...
  for (const item of validatedItems) {
    ...
    `, [
      orderRow.id, item.product, item.name, item.price, item.quantity, item.image, item.color, item.size, item.name, item.image, item.price, item.seller_id
    ]);
  ...
  await db.query(`
    INSERT INTO payments (
      order_id, user_id, amount, currency, method, status, gateway, created_at, updated_at
    ) VALUES (
      $1, $2, $3, 'INR', $4, $5, $6, now(), now()
    )
  `, [
    orderRow.id, req.user._id.toString(), totalAmount, selectedPayment, paymentStatus, selectedPayment === 'cod' ? '' : 'Razorpay'
  ]);
  ...
  const shipmentResult = await db.query(`
    INSERT INTO shipments (
      order_id, shipment_id, delivery_mode, status, origin, destination, otp_code, created_at, updated_at
    ) VALUES (
      $1, $2, 'road', 'Shipment Request', '{"address": "Warehouse"}'::jsonb, $3, 'OTP-' || floor(random() * 900000 + 100000)::text, now(), now()
    ) RETURNING *
  `, [orderRow.id, trackingNumber, JSON.stringify(shippingAddress)]);
  ...
  await db.query(`
    INSERT INTO shipment_events (
      ...
      $1, 'Shipment Request', 'Order placed successfully and awaiting pickup', now()
    )
...
exports.trackByNumber = asyncHandler(async (req, res) => {
  const shipmentRes = await db.query("SELECT * FROM shipments WHERE shipment_id = $1", [req.params.trackingNumber]);
  if (shipmentRes.rows.length === 0) {
    return res.status(404).json({ success: false, message: 'Shipment not found' });
  }
  const shipment = shipmentRes.rows[0];
