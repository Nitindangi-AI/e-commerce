  ...
  return (
    <div className="max-w-7xl mx-auto px-6 pt-28 pb-20 min-h-screen">
      ...
      <div className="flex items-center gap-2 text-xs text-[var(--text-secondary)] tracking-wider mb-8">
        ...
        <span>/</span>
        <span className="text-[var(--text-primary)]">Wishlist</span>
      </div>

      <div className="flex items-center justify-between mb-10">
        <h1 className="display text-4xl font-black text-[var(--text-primary)]">Your Wishlist</h1>
        {wishlistItems.length > 0 && (
          <button onClick={clearWishlist}
            className="text-xs tracking-widest uppercase text-[var(--text-secondary)] hover:text-red-500 transition-colors">
            Clear All
          </button>
        )}
      </div>
